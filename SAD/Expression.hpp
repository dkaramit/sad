...
class Expression{
    ...
    Expression<numType> subExpr()const{return expr_ptr->subExpr();}
...
template<typename numType>
Expression<numType> Expression<numType>::derivative(const IDType &wrt)const{ return wrt==0? ZERO<numType> : expr_ptr->derivative(wrt); }
